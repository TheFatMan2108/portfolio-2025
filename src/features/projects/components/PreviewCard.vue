...
import { enabledSocialLinks, getProjectThemeStyle } from "../../../content/portfolio";
...
const contactUrl = computed(
  () =>
    enabledSocialLinks.value.find((item) => item.icon === "mail" || item.url.startsWith("mailto:"))?.url ?? "#contact",
);
const projectThemeStyle = computed(() => (props.preview ? getProjectThemeStyle(props.preview.slug) : {}));
...
.preview-card {
  ...
  &-title {
    font-family: var(--portfolio-font-project-card-title, var(--portfolio-font-family, "Urbanist", Arial, sans-serif));
    font-size: var(--font-size-title-xs);
    font-weight: 700;
    color: var(--project-title-color, var(--color-text-400));
  }

  &-description {
    font-family: var(
      --portfolio-font-project-card-description,
      var(--portfolio-font-family, "Urbanist", Arial, sans-serif)
    );
    font-size: var(--font-size-md);
    color: var(--project-description-color, var(--color-text-300));
    font-weight: 500;
  }
}
</style>
